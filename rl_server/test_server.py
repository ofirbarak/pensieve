            ...
            if "pastThroughput" in post_data:
                ...
                for i in range(A_DIM):
                    next_video_chunk_sizes.append(
                        get_chunk_size(i, self.input_dict["video_chunk_coount"])
                    )
                ...
                try:
                    ...
                    state[3, -1] = (
                        float(video_chunk_fetch_time) / M_IN_K / BUFFER_NORM_FACTOR
                    )  # 10 sec
                    state[4, :A_DIM] = (
                ...
                bit_rate = bytes(str("5"), "utf-8") #TODO: change to actual bitrate
...
def main():
    if len(sys.argv) == 2:
        ...
        run(log_file_path=LOG_FILE + "_TEST_" + trace_file)
    else:
        run()
